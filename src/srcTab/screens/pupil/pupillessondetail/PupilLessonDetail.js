...
const PupilLessonDetail = (props) => {
  ...
  const getHomeworkData = (searchBy, filterBy) => {
    ...
    Service.post(
      data,
      `${EndPoints.GetAllHomeworkListByPupil}/${User.user.UserDetialId}`,
      (res) => {
  ...
  const getLessonData = (searchBy, filterBy) => {
    ...
    Service.post(
      data,
      `${EndPoints.GetAllPupilLessonList}/${User.user.UserDetialId}`,
      (res) => {
        ...
        res.data.map((item) => {
          ...
          if (
            startDate.format("DD/MM/YYYY") <= date &&
            endDate.format("DD/MM/YYYY") >= date
          ) {
            current.push(item);
          } else {
            last.push(item);
        ...
        >
          {isLesson ? (
            <PupilLesson
              ...
              navigatePupilLessonDetailInternal={(item) => {
                setItem(item), setLessonDetail(true);
              }}
              isLoading={isLoading}
            />
          ) : (
            <PupilLessonDue
              ...
              navigatePupilHomeworkemarked={(item) => {
