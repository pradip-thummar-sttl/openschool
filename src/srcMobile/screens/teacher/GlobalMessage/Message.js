...
const Message = (props) => {
    ...
    return (
      <View style={PAGESTYLE.searchParent}>
        <View style={PAGESTYLE.searchInner}>
          <TouchableOpacity activeOpacity={opacity} onPress={() => {
              search();
            }}>
      ...
      >
        <View style={PAGESTYLE.flateMainView}>
          ...
          <View style={PAGESTYLE.secondRow}>
            <Text
              numberOfLines={1}
              style={[PAGESTYLE.titleText, { width: "85%" }]}
            >
              {item.item.Title}
            </Text>
            {/* <Image style={PAGESTYLE.pupilDetaillinkIcon} source={Images.DashboardRightArrow} /> */}
            <ArrowNext
              style={PAGESTYLE.pupilDetaillinkIcon}
              height={hp(1.54)}
              width={hp(1.54)}
            />
          </View>
  ...
  const messageRender = ({ item, index }) => {
    return <MessageList item={item} navigateToDetail={() => {}} />;
  };
  ...
  const fetchRecord = (searchby, filterBy) => {
    ...
    Service.post(
      data,
      `${EndPoints.GlobalMessaging}/${User.user._id}/T`,
      (res) => {
        setLoading(false);
        if (res.code == 200) {
    ...
    <View>
      ...
      {isLoading ? (
        ...
        <EmptyStatePlaceHohder
          holderType={2}
          title1={MESSAGE.noMessage1}
          title2={MESSAGE.noMessage2}
        />
      )}
    </View>
  );
